...
class HtmlToMarkdown extends ControllerAbstract {
	...
	public function admin_init() {
		...
		if ( array_intersect( $allowed_roles, $user->roles ) || is_super_admin() ) {
			...
			add_action( 'wp_ajax_future_html2markdown', array( $this, 'admin_future_html2markdown' ) );
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		wp_enqueue_script( 'future-md-h2m', $this->future_plugin_url . 'assets/js/future-md-h2m.js', array(), $this->version, true );

		$data['ajax_url'] = admin_url( 'admin-ajax.php' );
		$data['post_id']  = future_get_current_post_id();

		wp_localize_script( 'future-md-h2m', 'h2m_config', $data );
	}
	...
	public function add_meta_box() {
		...
		add_meta_box(
			...
			__( 'HTML to Markdown', 'wp-future-md' ),
	...
	public function show_meta_box() {
		echo future_load_view( 'metabox/html-to-markdown' );
	}

	/**
	 * Do action hook for image paste.
	 */
	public function admin_future_html2markdown() {
		$is_strip_tags = false;
		$is_line_break = false;
		$post_content  = '';
